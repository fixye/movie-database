(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))c(e);new MutationObserver(e=>{for(const i of e)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&c(l)}).observe(document,{childList:!0,subtree:!0});function s(e){const i={};return e.integrity&&(i.integrity=e.integrity),e.referrerPolicy&&(i.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?i.credentials="include":e.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function c(e){if(e.ep)return;e.ep=!0;const i=s(e);fetch(e.href,i)}})();const v="a79bbef9",p=document.querySelector("input"),C=document.querySelector(".search-btn"),r=document.querySelector(".movies"),f=document.querySelector(".tool-bar"),d=document.querySelector("h1"),h=document.querySelector("h3");let o,a=[];const w=t=>{o.push(a[t]),localStorage.setItem("watchlist",JSON.stringify(o))};window.addToWatchlist=w;window.deleteItemFromWatchlist=L;h.addEventListener("click",y);d.addEventListener("click",u);p.addEventListener("keypress",t=>{t.key==="Enter"&&m()});C.addEventListener("click",m);u();function u(){d.textContent="Find your film",h.textContent="My Watchlist",f.classList.remove("hidden"),r.innerHTML=`
        ...
        `,H()}function H(){localStorage.getItem("watchlist")?o=JSON.parse(localStorage.getItem("watchlist")):o=[]}async function m(){let t="";a.length=0;const s=await(await fetch(`https://www.omdbapi.com/?apikey=${v}&s=${p.value}&type=movie&r=json`)).json();if(s.Response==="True"){let c=0;for(const e of s.Search){r.innerHTML="";try{const l=await(await fetch(`https://www.omdbapi.com/?apikey=${v}&i=${e.imdbID}`)).json();t+=`<article class="movie">
                ...
                </article>`,r.innerHTML+=r.innerHTML+t,c++,a.push(l)}catch(i){console.log(i.message)}}}else r.innerHTML=`
        <div class="default-state">
            <p>No data found for your query. Please try again.</p>
        </div>
        `}function y(){d.textContent="My Watchlist",h.textContent="Search for movies",f.classList.add("hidden"),g()}function g(){let t="",n=0;o.length?(o=JSON.parse(localStorage.getItem("watchlist")),o.forEach(s=>{t+=`<article class="movie">
            <img src="${s.Poster}" alt="${s.Title} Poster">
            <div>
                <div class="movie-head">
                <h4 ${s.length>60&&'class="smaller"'}>${s.Title}</h4>
                <small class="score"><svg width="13" height="12" viewBox="0 0 13 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M5.78671 1.19529C6.01122 0.504306 6.98878 0.504305 7.21329 1.19529L8.01547 3.66413C8.11588 3.97315 8.40384 4.18237 8.72876 4.18237H11.3247C12.0512 4.18237 12.3533 5.11208 11.7655 5.53913L9.66537 7.06497C9.40251 7.25595 9.29251 7.59447 9.39292 7.90349L10.1951 10.3723C10.4196 11.0633 9.62875 11.6379 9.04097 11.2109L6.94084 9.68503C6.67797 9.49405 6.32203 9.49405 6.05916 9.68503L3.95903 11.2109C3.37125 11.6379 2.58039 11.0633 2.8049 10.3723L3.60708 7.90349C3.70749 7.59448 3.59749 7.25595 3.33463 7.06497L1.2345 5.53914C0.646715 5.11208 0.948796 4.18237 1.67534 4.18237H4.27124C4.59616 4.18237 4.88412 3.97315 4.98453 3.66414L5.78671 1.19529Z" fill="#FEC654"/>
                    </svg>${s.imdbRating}</small>
                </div>
                <div class="movie-details">
                <p class="length">${s.Runtime}</p>
                <p class="genres">${s.Genre}</p>
        ...
        `,r.innerHTML=t,document.querySelector("a").addEventListener("click",u))}function L(t){o.splice(t,1),localStorage.setItem("watchlist",JSON.stringify(o)),g()}
