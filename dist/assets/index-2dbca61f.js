(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))c(e);new MutationObserver(e=>{for(const l of e)if(l.type==="childList")for(const s of l.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&c(s)}).observe(document,{childList:!0,subtree:!0});function i(e){const l={};return e.integrity&&(l.integrity=e.integrity),e.referrerPolicy&&(l.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?l.credentials="include":e.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function c(e){if(e.ep)return;e.ep=!0;const l=i(e);fetch(e.href,l)}})();const v="a79bbef9",p=document.querySelector("input"),C=document.querySelector(".search-btn"),r=document.querySelector(".movies"),f=document.querySelector(".tool-bar"),d=document.querySelector("h1"),h=document.querySelector("h3");let o,a=[];function w(t){o.push(a[t]),localStorage.setItem("watchlist",JSON.stringify(o))}window.addToWatchlist=w;window.deleteItemFromWatchlist=y;h.addEventListener("click",L);d.addEventListener("click",u);p.addEventListener("keypress",t=>{t.key==="Enter"&&m()});C.addEventListener("click",m);u();function u(){d.textContent="Find your film",h.textContent="My Watchlist",f.classList.remove("hidden"),r.innerHTML=`
        ...
        `,H()}function H(){localStorage.getItem("watchlist")?o=JSON.parse(localStorage.getItem("watchlist")):o=[]}async function m(){let t="";a.length=0;const i=await(await fetch(`https://www.omdbapi.com/?apikey=${v}&s=${p.value}&type=movie&r=json`)).json();if(i.Response==="True"){let c=0;console.log(a);for(const e of i.Search){r.innerHTML="";const s=await(await fetch(`http://www.omdbapi.com/?apikey=${v}&i=${e.imdbID}`)).json();t+=`<article class="movie">
            <img src="${s.Poster}" alt="${s.Title} Poster">
            <div>
                <div class="movie-head">
                <h4 ${s.Title.length>60&&'class="smaller"'}>${s.Title}</h4>
        ...
        </article>`,r.innerHTML+=r.innerHTML+t,c++,a.push(s)}}}function L(){d.textContent="My Watchlist",h.textContent="Search for movies",f.classList.add("hidden"),g()}function g(){let t="",n=0;o.length?(o=JSON.parse(localStorage.getItem("watchlist")),o.forEach(i=>{console.log(i),t+=`<article class="movie">
